...
<div class="p-8 bg-gray-50 min-h-screen">
    <div class="max-w-4xl mx-auto bg-white border border-gray-200 rounded-md shadow-sm p-8">
        ...
        <form method="POST" action="/Cliente/Atualizar/{{ $Cliente['id'] }}" class="space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="observacoes" rows="4"
                        class="w-full border border-gray-300 bg-gray-50 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-gray-800 rounded-sm px-3 py-2.5 text-sm"
                        placeholder="Informações adicionais do cliente...">{{ $Cliente['observacoes'] }}</textarea>
                </div>
            </div>

            <div class="flex justify-between mt-8">
                <a href="/Cliente/Listar" class="bg-gray-500 hover:bg-gray-600 text-white font-medium px-6 py-2.5 text-sm rounded-sm transition">
                    Cancelar
                </a>
                <button type="submit"
